  ...
  /*--- Check for a restart, initialize from zero otherwise ---*/
	...
	if (!restart) {
	    ...
	    for(iMarker = 0; iMarker < nMarker; iMarker++) {
	    	unsigned long nElBound;
	    	nElBound = geometry->GetnElem_Bound(iMarker);
	        element[iMarker] = new CFEABoundVariable(nDim, nVar, nElBound, config);
	    }
	}
	else {
		unsigned long index;
...
void CFEASolver::SetFEA_Load(CSolver ***flow_solution, CGeometry **fea_geometry, CGeometry **flow_geometry,
	...
	else if (strucRelaxFactFSI < 0.0 ){
		cout << "Structural relaxation factor can't be under 0. Setting it to 0. " << endl;
		strucRelaxFactFSI=0;
	}
